...
- Follow-up (260925): MiMo V2 can send `{}` for a declared freeform call while placing its code only in a standalone bare block, sometimes after a malformed `<parameter=` opener. One structured call to the exact wire tool authorizes recovering that body as `input` only when no other answer text was released; other models, prose, ambiguous responses, and quoted or fenced markup retain the original match-only behavior. This repairs a missing executable input, not just a visible echo.
